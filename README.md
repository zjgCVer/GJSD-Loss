...
In oriented object detection for remote sensing images, Gaussian-based bounding box regression maps an oriented bounding box to a two-dimensional Gaussian distribution, enabling a unified probabilistic formulation for jointly modeling position, scale, and orientation. Its effectiveness, however, largely depends on the metric used to compare the predicted and target distributions. Existing metrics exhibit different limitations: the Kullback--Leibler divergence is direction-dependent, the Gaussian Wasserstein distance has scale-dependent responses, and probability-overlap-based measures may provide weakened supervision in large-error regions. This paper proposes a geometric Jensen--Shannon divergence loss, termed GJSD Loss, for oriented bounding box regression. By introducing a normalized geometric mixture that combines the input Gaussians in the precision-matrix domain, i.e., through their inverse covariance matrices, GJSD yields a closed-form divergence between multivariate Gaussian distributions and jointly measures center offsets, size/shape variations, and orientation errors. The resulting loss is symmetric, non-negative, has a unique zero point, and is scale-invariant. It can be directly used as a regression-loss replacement in existing oriented detectors. Under a controlled replacement protocol, experiments on multiple detectors and datasets show that GJSD Loss achieves competitive overall accuracy and more evident gains on high-threshold localization metrics. In particular, on DOTA-v1.0, S<sup>2</sup>A-Net+GJSD and RoI Transformer+GJSD achieve 79.66%/58.47% and 80.56%/56.23% in mAP/AP<sub>75</sub>, respectively, while RoI Transformer+GJSD achieves 66.02\% mAP on DIOR-R. These results show that GJSD is particularly useful for high-threshold oriented localization. 
